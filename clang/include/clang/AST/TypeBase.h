...
enum class DeducedKind {
  /// This has not been deduced yet. This is for example an 'auto' which was
  /// just parsed.
  Undeduced,
  ...
  /// Formally we can't deduce a type which is dependent, because for one reason
  /// it might be non-instantiable (ie it can contain a placeholder dependent
  /// type such as DependentTy, which cannot be instantiated).
