...
CGCXXABI::RecordArgABI CodeGen::getRecordArgABI(const RecordDecl *Rec,
                                                CGCXXABI &CXXABI) {
  const CXXRecordDecl *RD = dyn_cast<CXXRecordDecl>(Rec);
  if (!RD) {
    if (!Rec->canPassInRegisters())
...
CGCXXABI::RecordArgABI CodeGen::getRecordArgABI(QualType T, CGCXXABI &CXXABI) {
  ...
  if (!RD)
    return CGCXXABI::RAA_Default;
  return getRecordArgABI(RD, CXXABI);
}
...
QualType CodeGen::useFirstFieldIfTransparentUnion(QualType Ty) {
  if (const RecordDecl *UD = Ty->getAsUnionDecl()) {
...
bool CodeGen::isEmptyField(ASTContext &Context, const FieldDecl *FD,
  ...
  const RecordDecl *RD = FT->getAsRecordDecl();
  if (!RD)
    return false;
