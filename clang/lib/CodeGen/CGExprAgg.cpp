...
bool AggExprEmitter::TypeRequiresGCollection(QualType T) {
  ...
  const RecordDecl *Record = T->getAsRecordDecl();
  if (!Record)
    return false;
...
void AggExprEmitter::VisitCXXParenListOrInitListExpr(
  ...
  RecordDecl *record = ExprToVisit->getType()->getAsRecordDecl();
...
static CharUnits GetNumNonZeroBytesInInit(const Expr *E, CodeGenFunction &CGF) {
  ...
  if (const RecordDecl *SD = E->getType()->getAsRecordDecl()) {
    if (!SD->isUnion()) {
...
static void CheckAggExprForMemSetUse(AggValueSlot &Slot, const Expr *E,
  ...
  if (CGF.getLangOpts().CPlusPlus)
    if (const CXXRecordDecl *RD = CGF.getContext()
...
void CodeGenFunction::EmitAggregateCopy(LValue Dest, LValue Src, QualType Ty,
  ...
  if (CGM.getLangOpts().getGC() == LangOptions::NonGC) {
    // fall through
  } else if (const RecordDecl *Record = Ty->getAsRecordDecl()) {
    ...
    if (const RecordDecl *Record = BaseType->getAsRecordDecl()) {
      if (Record->hasObjectMember()) {
        CGM.getObjCRuntime().EmitGCMemmoveCollectable(*this, DestPtr, SrcPtr,
                                                      SizeVal);
        return;
