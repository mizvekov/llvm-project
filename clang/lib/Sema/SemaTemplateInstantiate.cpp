...
MultiLevelTemplateArgumentList Sema::getTemplateInstantiationArgs(
  ...
  while (!NumLevels || *NumLevels != 0U) {
    ...
    case Decl::VarTemplateSpecialization: {
      assert(!Innermost);
      const auto *ND = cast<VarTemplateSpecializationDecl>(D);
    ...
    case Decl::ClassTemplateSpecialization: {
      assert(!Innermost);
      const auto *ND = cast<ClassTemplateSpecializationDecl>(D);
      if (ND->getSpecializationKind() == TSK_ExplicitSpecialization) {
        assert(TPLs.empty());
        break;
      }
    ...
    case Decl::ImplicitConceptSpecialization:
      ...
      break;
    case Decl::Var: {
      const auto *ND = cast<VarDecl>(D);
      if (const auto *TD = ND->getDescribedVarTemplate()) {
        D = TD;
        continue;
      }
      assert(!Innermost);
    ...
    case Decl::CXXMethod:
    case Decl::Function: {
      const auto *ND = cast<FunctionDecl>(D);
      if (const auto *TD = ND->getDescribedFunctionTemplate()) {
        D = TD;
        continue;
      }
      assert(!Innermost);
    ...
    case Decl::CXXRecord: {
      const auto *ND = cast<CXXRecordDecl>(D);
      if (const auto *TD = ND->getDescribedClassTemplate()) {
        D = TD;
        continue;
      }
      assert(!Innermost);
      // A lambda can have a template parent which is not a DeclContext, such as
      // a type alias template.
      if (ND->isLambda()) {
        assert(TPLs.empty() &&
               "lambdas can't be part of an out-of-line definition");
        // FIXME: If this lambda is enclosed by a type alias template, look into
        // the current CodeSynthesisContexts to find the template arguments used
        // to specialize it.
    ...
    case Decl::VarTemplate:
      if (const auto *ND = cast<TemplateDecl>(D); !Innermost) {
        Innermost =
            ND->getTemplateParameters()->getInjectedTemplateArgs(Context);
      } else {
        SkipInnerNonInstantiated = false;
        Final = true;
      }
    ...
    case Decl::ClassTemplatePartialSpecialization:
      if (const auto *ND = cast<ClassTemplatePartialSpecializationDecl>(D);
          !Innermost) {
        Innermost =
            ND->getTemplateParameters()->getInjectedTemplateArgs(Context);
      } else {
    ...
    case Decl::VarTemplatePartialSpecialization:
      if (const auto *ND = cast<VarTemplatePartialSpecializationDecl>(D);
          !Innermost) {
        Innermost =
            ND->getTemplateParameters()->getInjectedTemplateArgs(Context);
      } else {
    ...
    default:
      D->dumpColor();
      llvm_unreachable("unexpected decl kind in template argument collection");
      break;
    };
    ...
    case NestedNameSpecifier::Kind::Global:
      ...
      break;
    case NestedNameSpecifier::Kind::Type:
      if (const auto *T = Qualifier.getAsType();
