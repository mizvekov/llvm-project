  ...
  if (Function->isLocalExternDecl()) {
    for (ParmVarDecl *PVD : Function->parameters()) {
      if (!PVD->hasDefaultArg())
        continue;
      // SemaRef.InstantiateDefaultArgument(D->getInnerLocStart(), Function,
      // PVD);
...
void Sema::InstantiateExceptionSpec(SourceLocation PointOfInstantiation,
  ...
  if (const auto *Info = Decl->getTemplateSpecializationInfo();
      ...
      Info->getTemplateSpecializationKind() == TSK_ExplicitSpecialization)
    TemplateArgs = getTemplateInstantiationArgs(
        Info->getTemplate(), Info->TemplateArguments->asArray());
  else
    TemplateArgs = getTemplateInstantiationArgs(Decl);
...
void Sema::InstantiateFunctionDefinition(SourceLocation PointOfInstantiation,
  ...
  if (PatternDecl->isDefaulted()) {
    ...
    if (const FunctionTemplateSpecializationInfo *Info =
      ...
      if (!TemplatedDecl->isInstantiatedFromMemberTemplate() &&
          !TemplatedDecl->isThisDeclarationADefinition()) {
        auto Redecls = TemplatedDecl->redecls();
        auto It = llvm::find_if(Redecls, [](const FunctionDecl *FD) {
          // return FD->isCompatibleWithDefinition();
      ...
      assert(Function->getTemplateSpecializationKind() !=
             ...
             "Expected to find an implicit instantiation");
    }
