...
static bool tryMatchRecordTypes(ASTContext &Context,
  ...
  RecordDecl *left = lt->getAsRecordDecl();
  RecordDecl *right = rt->getAsRecordDecl();
  if (!left || !right)
    return false;
...
static bool IsVariableSizedType(QualType T) {
  ...
  return (Record && Record->hasFlexibleArrayMember());
}
...
static void DiagnoseVariableSizedIvars(Sema &S, ObjCContainerDecl *OCD) {
  ...
  for (ObjCIvarDecl *ivar = IntfDecl->all_declared_ivar_begin(); ivar;
    ...
    if (IvarTy->isIncompleteArrayType()) {
      ...
      IsInvalidIvar = true;
    } else if (const RecordDecl *Record = IvarTy->getAsRecordDecl()) {
...
void SemaObjC::SetIvarInitializers(ObjCImplementationDecl *ObjCImplementation) {
  ...
  if (ObjCInterfaceDecl *OID = ObjCImplementation->getClassInterface()) {
    ...
    for (unsigned i = 0; i < ivars.size(); i++) {
      ...
      if (CXXRecordDecl *RD = Context.getBaseElementType(Field->getType())
                                  ->getAsCXXRecordDecl()) {
